...
static void add_disk_shashlik(Detector& desc, Assembly& env, xml::Collection_t& plm, SensitiveDetector& sens, int sid)
{
  ...
  auto points = (nsides == 6) ? D2EIC::geo::fillHexagons({0., 0.}, sidelen, rmin, rmax, phimin, phimax)
                              : D2EIC::geo::fillSquares({0., 0.}, sidelen * 1.414, rmin, rmax, phimin, phimax);
  // placement to mother
...
DECLARE_DETELEMENT(D2EIC_ShashlikCalorimeter, create_detector)
