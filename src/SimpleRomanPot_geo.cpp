...
// Modified for simple roman pot detector Jihee Kim
...
static Ref_t create_detector(Detector& desc, xml_h handle, SensitiveDetector sens)
{
  ...
  Volume env(detName + "_envelope", envShape, desc.material("Vacuum"));
...
DECLARE_DETELEMENT(ip8_SimpleRomanPot, create_detector)
